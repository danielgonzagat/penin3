...
# -*- coding: utf-8 -*-
"""
PENIN-Ω v6.0 FUSION - CÓDIGO 3/8: Aquisição de Conhecimento & Contexto
================================================================================
...
class KnowledgeAcquisitionEngine:
    ...
    async def execute(self) -> AcquisitionReport:
        ...
        # CORREÇÃO: Atualiza estado evolutivo (↓ é melhor para ambas as métricas)
        self.X.novelty_sim = min(self.X.novelty_sim, novelty_sim)  # ↓ evolutivo
        self.X.rag_recall = min(self.X.rag_recall, rag_recall)    # ↓ evolutivo
        ...
        # CORREÇÃO: Validação de performance sub-10ms
        cycle_time_ms = (time.time() - start_time) * 1000
        if cycle_time_ms > 10:
            log(f"Ciclo 3/8 demorou {cycle_time_ms:.1f}ms (>10ms target)", level="WARNING")
        ...
        self.worm.record_event(
            AcquisitionEventType.ACQ_DONE.value,
            {
                ...
                "processing_time": cycle_time_ms,
                "performance_ok": cycle_time_ms <= 10
            }
        )
...
if __name__ == "__main__":
    ...
    async def run_test():
        ...
        logger.info("\n" + "="*60)
        logger.info("RELATÓRIO DE AQUISIÇÃO - FASE 3")
        logger.info("="*60)
        ...
        logger.info("="*60)
        logger.info(f"novelty_sim: {test_state.novelty_sim:.3f} (↓ é melhor)")
        logger.info(f"rag_recall: {test_state.rag_recall:.3f} (↓ é melhor)")
        logger.info(f"hashes: {len(test_state.hashes)} registrados")
        logger.info(f"proof_ids: {len(test_state.proof_ids)} provas")
