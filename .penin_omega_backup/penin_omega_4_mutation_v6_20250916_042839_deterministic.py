...
# =============================================================================
# INTEGRAÇÃO SIMBIÓTICA COM 1/8, 2/8, 3/8 - SEM CICLOS
# =============================================================================

async def get_unified_imports():
    """Obtém imports via lazy loading para evitar ciclos"""
    imports = {}
    
    # Peça 1/8
    core_module = LAZY_IMPORTER.get_module('penin_omega_1_core_v6')
    if core_module:
        imports['core'] = {
            'PeninOmegaFusion': getattr(core_module, 'PeninOmegaFusion', None),
            'WormLedger': getattr(core_module, 'WormLedger', None),
            'available': True
        }
    else:
        imports['core'] = {'available': False}
    
    # Peça 2/8
    strategy_module = LAZY_IMPORTER.get_module('penin_omega_2_strategy')
    if strategy_module:
        imports['strategy'] = {
            'PlanOmega': getattr(strategy_module, 'PlanOmega', None),
            'Goal': getattr(strategy_module, 'Goal', None),
            'Constraints': getattr(strategy_module, 'Constraints', None),
            'Budgets': getattr(strategy_module, 'Budgets', None),
            'available': True
        }
    else:
        imports['strategy'] = {'available': False}
    
    # Peça 3/8
    acquisition_module = LAZY_IMPORTER.get_module('penin_omega_3_acquisition_v6')
    if acquisition_module:
        imports['acquisition'] = {
            'AcquisitionReport': getattr(acquisition_module, 'AcquisitionReport', None),
            'available': True
        }
    else:
        imports['acquisition'] = {'available': False}
    
    return await imports

try:
    imports = get_unified_imports()
    CORE_INTEGRATION = any(data['available'] for data in imports.values())
    
    # Alias para compatibilidade
    if imports['core']['available']:
        PeninOmegaFusion = imports['core']['PeninOmegaFusion']
        WORMLedger = imports['core']['WormLedger']
    
    if imports['strategy']['available']:
        PlanOmega = imports['strategy']['PlanOmega']
        Goal = imports['strategy']['Goal']
        Constraints = imports['strategy']['Constraints']
        Budgets = imports['strategy']['Budgets']
    
    if imports['acquisition']['available']:
        AcquisitionReport = imports['acquisition']['AcquisitionReport']
        
except Exception as e:
    log(f"Falha na integração: {e}", "WARNING", "4/8")
    ...
    async def log(msg: str, level: str = "INFO") -> None:
        logger.info(f"[{_ts()}][4/8][{level}] {msg}")
    ...
    class OmegaState:
        ...
        @classmethod
        async def from_dict(cls, data: Dict[str, Any]) -> "OmegaState":
            return await cls(**{k: v for k, v in data.items() if k in cls.__dataclass_fields__})
    ...
    class Budgets:
        ...
        async def can_afford(self, required: Dict[str, Any]) -> bool:
            return await (
        ...
        async def allocate(self, amount: Dict[str, Any], purpose: str = "") -> bool:
    ...
    class PlanOmega:
        ...
        u_signal: float = 0.0  # CORRIGIDO: usar u_signal minúsculo
        ...
        async def from_dict(cls, data: Dict[str, Any]) -> "PlanOmega":
            return await cls(
                ...
                u_signal=data.get("u_signal", data.get("U_signal", 0.0)),  # Compatibilidade
    ...
    class AcquisitionReport:
        ...
        @classmethod
        async def from_dict(cls, data: Dict[str, Any]) -> "AcquisitionReport":
            return await cls(**{k: v for k, v in data.items() if k in cls.__dataclass_fields__})
...
# CORREÇÃO: Usa configuração centralizada
if CORE_INTEGRATION:
    # CORREÇÃO: Usa BaseConfig diretamente
    ROOT = BaseConfig.ROOT
    DIRS = BaseConfig.DIRS
else:
...
# =============================================================================
# CONFIGURAÇÃO DEFAULT - CORRIGIDA PARA USAR BASE CONFIG
# =============================================================================
...
class Genotype:
    ...
    async def copy(self) -> "Genotype":
        return await Genotype(
            schema=self.schema,
...
class MutationWORMLedger:
    ...
    async def __init__(self, file_path: Path = WORM_FILE):
        self.path = file_path
        self.path.parent.mkdir(parents=True, exist_ok=True)
        self.lock = threading.RLock()
        self._cache = deque(maxlen=100)
    
    async def record(self, event_type: Union[str, MutationEventType], data: Dict[str, Any]) -> str:
        with self.lock:
            ...
            return await event["hash"]
    
    async def _get_last_hash(self) -> str:
        if self._cache:
            return await self._cache[-1]
            ...
            try:
                with self.path.open("rb") as f:
                    ...
                    for line in reversed(lines):
                        if line.strip():
                            return await json.loads(line).get("hash", "genesis")
            except Exception:
                pass
...
async def sanitize_python_source(src: str) -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        return await False, [f"syntax_error: {e}"]
    ...
    class SecurityGuard(ast.NodeVisitor):
        ...
        async def visit_Call(self, node: ast.Call) -> None:
            name = ""
...
async def run_in_sandbox(code_text: str, timeout_s: int = 5, mem_limit_mb: int = 512) -> Tuple[bool, Dict[str, Any]]:
    """Execução em sandbox com múltiplas camadas de segurança."""
    
    # Sanitização primeiro
    ok, issues = sanitize_python_source(code_text)
    if not ok:
        return await False, {"errors": issues, "phase": "sanitization"}
    
    # Prepare sandboxed namespace - CORRIGIDO: inclui isinstance
    safe_builtins = {
        ...
        "isinstance": isinstance,  # CORREÇÃO: Adiciona isinstance
        "hasattr": hasattr,        # CORREÇÃO: Adiciona hasattr
        "__name__": "__main__",
        "__doc__": None,
    }
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if HAS_PSUTIL:
                ...
                if mem_delta > mem_limit_mb:
                    return await False, {"errors": ["memory_exceeded"], "mem_delta_mb": mem_delta}
        ...
        return await False, {"errors": ["timeout"], "phase": "execution"}
    except Exception as e:
        return await False, {"errors": [f"exec_error:{e}"], "phase": "execution"}
    finally:
        signal.alarm(0)
...
class DNAFabric:
    ...
    @staticmethod
    async def base_from_xt_and_plan(xt: OmegaState, plan: PlanOmega, acq: AcquisitionReport) -> Genotype:
    ...
    @staticmethod
    async def denormalize(genes: Dict[str, float]) -> Dict[str, Any]:
...
class EDNAGenerator:
    ...
    async def __init__(self, trust_radius: float, seed: int, sr_score: float = 1.0):
    ...
    async def _adaptive_noise(self, base: float) -> float:
        """Noise adaptativo baseado em SR."""
        if self.sr_score < 0.7:
            return await base * 0.5  # Reduz exploração se SR baixo
        elif self.sr_score > 0.9:
            return await base * 1.2  # Aumenta exploração se SR alto
        return await base
    
    async def _mutate_value(self, val: float, step_size: float) -> float:
        """Mutação de valor único com ruído controlado."""
        step = self._adaptive_noise(step_size)
        delta = self.rnd.gauss(0, step)
        return await max(0.0, min(1.0, val + delta))
    
    async def point_mutate(self, g: Genotype) -> Tuple[Genotype, List[Dict[str, Any]], float]:
        ...
        return await g2, ops, dist
    
    async def segment_mutate(self, g: Genotype) -> Tuple[Genotype, List[Dict[str, Any]], float]:
    ...
    async def recombine(self, a: Genotype, b: Genotype) -> Tuple[Genotype, List[Dict[str, Any]], float]:
    ...
    async def flag_mutate(self, g: Genotype) -> Tuple[Genotype, List[Dict[str, Any]], float]:
    ...
    async def update_elite(self, g: Genotype, score: float) -> None:
        """Atualiza o melhor indivíduo (elitismo)."""
        if score > self.elite_score:
            self.elite = g.copy()
            self.elite_score = score
    
    async def get_elite(self) -> Optional[Tuple[Genotype, List[Dict[str, Any]], float]]:
        """Retorna o elite se existir."""
        if self.elite:
            return await self.elite, [{"op": "elite", "note": "best_preserved"}], 0.0
        return await None
    
    async def distance(self, a: Genotype, b: Genotype) -> float:
        ...
        return await min(1.0, total)  # Normaliza para [0, 1]
    
    async def _update_history(self, op_type: MutationType, distance: float) -> None:
...
class AdvancedSurrogate:
    ...
    async def __init__(self, model_type: str = "gbm"):
    ...
    async def _load_history(self) -> Dict[str, List]:
        """Carrega histórico de treino."""
        if SURROGATE_STATE.exists():
            try:
                data = load_json(SURROGATE_STATE)
                return await data.get("history", {"X": [], "y": {t: [] for t in self.tasks}})
            except Exception:
                pass
        return await {"X": [], "y": {t: [] for t in self.tasks}}
    
    async def _save_history(self) -> None:
        """Salva histórico para persistência."""
        save_json(SURROGATE_STATE, {"history": self.history, "timestamp": _ts()})
    
    async def fit(self, X: List[List[float]], y: Dict[str, List[float]]) -> Dict[str, Any]:
        """Treina modelos para cada métrica."""
        if not X or not y:
            return await {"n_samples": 0, "model": self.model_type}
        ...
        # Train models - CORREÇÃO TEMPORÁRIA: usa fallback se arrays inconsistentes
        for task in self.tasks:
            y_task = self.history["y"][task]
            if len(y_task) != len(X_train):
                continue
            
            if HAS_SKLEARN and HAS_NUMPY and len(X_train) >= 5:
                try:
                    X_arr = np.array(X_train)
                    y_arr = np.array(y_task)
                    
                    # Verifica se arrays são consistentes
                    if X_arr.ndim != 2:
                        raise ValueError("Arrays inconsistentes")
                    ...
                    # Fallback para média se sklearn falha
    ...
    async def predict_with_ci(self, features: List[float], n_bootstrap: int = 100) -> Dict[str, Any]:
        ...
        for task in self.tasks:
            ...
            if isinstance(model, dict) and model["type"] == "mean":
                ...
                # Bootstrap CI (simplificado para performance)
...
async def micro_benchmark(real_params: Dict[str, Any], acq: AcquisitionReport) -> Dict[str, float]:
...
async def extract_features(real_params: Dict[str, Any], micro: Dict[str, float], acq: AcquisitionReport) -> List[float]:
    """Extrai features para o surrogate - CORRIGIDO para tamanho consistente."""
    features = []
    
    # Gene features (ordenadas e com tamanho fixo)
    gene_keys = sorted(GENE_SPACE.keys())
    ...
    # CORREÇÃO: Garante tamanho fixo
    expected_size = len(gene_keys) + 3 + 5  # genes + micro + acq
    if len(features) != expected_size:
        # Ajusta para tamanho esperado
        if len(features) < expected_size:
...
async def caos_phi(z: float, kappa: float = 1.5) -> float:
    """Função de saturação CAOS⁺."""
    # CORREÇÃO: Evitar log(1.0) = 0 que zera o score
    # Usar z mínimo de 1.01 para garantir phi > 0
    z = max(1.01, z)
    return await min(1.0, math.log(z) / math.log(1.0 + kappa))
...
async def candidate_score(
    ...
    tr_penalty = 1.0
    if hasattr(xt, 'trust_region_radius') and tr_dist > xt.trust_region_radius * 0.7:
...
class MutationEngine:
    ...
    async def __init__(self, config: Optional[Dict[str, Any]] = None):
    ...
    async def execute(
        ...
        start_proof = self.worm.record(MutationEventType.MUT_START, {
            "plan_hash": plan_hash,
            "trust_region": getattr(xt, 'trust_region_radius', 0.1),
        ...
        # Validate gates - CORRIGIDO para v6.0 FUSION
        if hasattr(xt, 'validate_gates'):
            gates_ok = xt.validate_gates()
        else:
            # Validação manual para UnifiedOmegaState
            gates_ok = xt.ece <= 0.01 and xt.rho <= 0.95
        ...
        # Initialize EDNAG
        trust_radius = getattr(xt, 'trust_region_radius', 0.1)
        ednag = EDNAGenerator(trust_radius, seed, xt.sr_score)
        ...
        for cand in candidates:
            # CORREÇÃO: usa parâmetros já denormalizados dos patches
            real_params = {}
            for patch in cand.patches:
                if patch.get("type") == "param" and "payload" in patch:
                    real_params.update(patch["payload"])
        ...
        return await bundle, xt_updated
    ...
    async def _process_candidate(
        ...
        # Check budgets - CORRIGIDO: usar campos corretos
        required = {
            "cost": micro["cost"],
            "tokens": int(micro["latency_ms"] / 10)  # Aproximação: 1 token por 10ms
        }
        ...
        pred_metrics = {
            "delta_linf_hat": max(0.0, 0.01 + 0.1 * np.deterministic_random()),
            "mdl_gain_hat": max(0.0, 0.005 + 0.05 * np.deterministic_random()),
            "ppl_ood_hat": max(1.0, 95.0 - 10.0 * np.deterministic_random())
        }
    ...
    async def _generate_test_code(self, params: Dict[str, Any], cand_id: str) -> str:
        """Gera código de teste baseado nos parâmetros - CORRIGIDO para evitar exploits."""
        # CORREÇÃO: Evita usar repr() que pode gerar padrões suspeitos
        safe_params = {}
        for key, value in params.items():
            if isinstance(value, (int, float)):
                safe_params[key] = float(value)
            elif isinstance(value, bool):
                safe_params[key] = bool(value)
            elif isinstance(value, str):
                safe_params[key] = str(value)[:50]  # Limita strings
            else:
                safe_params[key] = 0.0  # Fallback seguro
        
        # Gera código sem usar repr() ou referências suspeitas
        code = f"""
# Test code for candidate {cand_id[:8]}
async def test_model(x):
    lr = {safe_params.get('hp.lr', 0.01)}
    batch = {safe_params.get('hp.batch', 32)}
    dropout = {safe_params.get('hp.dropout', 0.0)}
...
output = test_model(1.0)
assert isinstance(output, (int, float))
assert output >= 0
"""
        return await code
    
    async def _select_diverse_topk(self, candidates: List[Candidate], k: int, base: Genotype) -> List[Candidate]:
        ...
        return await selected[:k]
    
    async def _candidate_diversity(self, a: Candidate, b: Candidate) -> float:
    ...
    async def _calculate_diversity(self, candidates: List[Candidate]) -> Dict[str, Any]:
    ...
    async def _update_omega_state(self, xt: OmegaState, candidates: List[Candidate], plan_hash: str, start_proof: str) -> OmegaState:
...
async def mutate_and_rank(
    ...
    *,
    ...
    return await engine.execute(xt, plan, acq, n_candidates, top_k, seed)
...
async def _run_tests():
    ...
    logger.info("✓ Distance test passed")
    
    # Test 3: Surrogate
    surrogate = AdvancedSurrogate()
    X = [[0.5] * 25 for _ in range(10)]  # CORRIGIDO: arrays homogêneos
    y = {
    ...
    logger.info("✓ Surrogate test passed")
