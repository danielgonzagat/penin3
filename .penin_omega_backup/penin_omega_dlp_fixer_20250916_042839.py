...
class DLPViolationFixer:
    ...
    async def add_violation_idempotent(self, violation_id: str, operation_id: str, content: str, 
        ...
        try:
            ...
            # Gera chave de operação
            operation_key = f"{violation_id}:{operation_id}:{content_hash}"
            ...
            try:
                ...
                cursor.execute('''
                    SELECT operation_id FROM idempotent_operations 
                    WHERE operation_key = ?
                ''', (operation_key_hash,))
                
                if cursor.fetchone():
                    # Operação já executada, retorna sucesso sem fazer nada
                    return await True
                
                # Tenta inserir violação
                timestamp = datetime.now(timezone.utc).isoformat()
                
                cursor.execute('''
                    INSERT OR IGNORE INTO dlp_violations 
