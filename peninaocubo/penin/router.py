...
class MultiLLMRouterComplete:
    ...
    def create_with_defaults(
        ...
        **kwargs: Any
        ...
        """
        Create router with default providers (factory classmethod).

        This automatically discovers available providers by trying to instantiate
        each provider class. Providers without API keys are skipped gracefully.

        Args:
            daily_budget_usd: Daily budget limit (default: $5.00)
        ...
        Returns:
            Configured router with available providers

        Example:
            >>> router = MultiLLMRouterComplete.create_with_defaults()
            >>> response = await router.route("Hello!")
        """
        from penin.providers import (
            AnthropicProvider,
            DeepSeekProvider,
            GeminiProvider,
            GrokProvider,
        ...
        # Try to instantiate each provider
        provider_classes = [
            OpenAIProvider,
            AnthropicProvider,
            GeminiProvider,
        ...
        for provider_class in provider_classes:
            try:
                ...
                # Provider initialization failed (no API key, etc)
                pass

        if not providers:
            # No providers available, create mock for testing
            class MockProvider(BaseProvider):
                def __init__(self):
                    super().__init__(name="mock", api_key="test")

                async def generate(self, prompt: str, **kwargs: Any) -> LLMResponse:
                    return LLMResponse(
                        content=f"Mock: {prompt[:50]}",
                        model="mock-v1",
                        tokens_used=10,
                        cost_usd=0.0,
                        latency_ms=1.0,
...
def create_router_with_defaults(
    ...
    **kwargs: Any
    ...
    """
    Factory function for creating router with default providers.

    This creates a router with all available providers that have API keys configured.
    ...
    Returns:
        Configured MultiLLMRouterComplete instance with available providers

    Example:
        >>> router = create_router_with_defaults(daily_budget_usd=10.0)
    ...
    # Try to instantiate each provider (will skip if no API key)
    provider_classes = [
        OpenAIProvider,
        AnthropicProvider,
        GeminiProvider,
    ...
    for provider_class in provider_classes:
        try:
            ...
            # Provider initialization failed (likely no API key)
            # Skip gracefully
            pass

    if not providers:
        # No providers available, create mock provider for testing
        from penin.providers.base import BaseProvider

        class MockProvider(BaseProvider):
            """Mock provider for testing when no API keys available."""
