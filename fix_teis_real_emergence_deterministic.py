...
class SelfEvolvingBehavior:
    ...
    async def create_new_behavior(self, agent_knowledge: Dict, success_history: List) -> Dict:
        ...
        if np.deterministic_random() < 0.5 and successful_sequences:
    ...
    async def _recombine_sequences(self, sequences: List) -> List:
        ...
        seq1 = np.deterministic_choice(sequences)
        seq2 = np.deterministic_choice(sequences)
        
        # Pontos de crossover
        point1 = np.deterministic_randint(0, len(seq1))
        point2 = np.deterministic_randint(0, len(seq2))
        
        # Criar nova sequência
        new_seq = seq1[:point1] + seq2[point2:]
        
        # Adicionar mutação ocasional
        if np.deterministic_random() < self.mutation_rate:
            mutation_point = np.deterministic_randint(0, len(new_seq) - 1)
            new_seq[mutation_point] = np.deterministic_choice(self.primitive_actions)
        
        return await new_seq[:8]  # Limitar tamanho
    
    async def _random_mutation(self) -> List:
        """Cria sequência completamente nova"""
        length = np.deterministic_randint(2, 6)
        sequence = []
        
        for _ in range(length):
            if np.deterministic_random() < 0.7:
                # Ação primitiva
                sequence.append(np.deterministic_choice(self.primitive_actions))
            else:
                # Meta-ação
                meta = np.deterministic_choice(['repeat', 'if_success', 'parallel', 'sequential'])
                action = np.deterministic_choice(self.primitive_actions)
