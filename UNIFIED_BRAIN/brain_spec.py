...
# Safe dtype mapping (avoid eval on untrusted input)
DTYPE_MAP = {
...
class NeuronMeta:
    ...
    tags: List[str] = None
    generation: int = 0  # ✅ Darwin generation tracking
    ...
    def load_registry(self, path: str):
        ...
        for nid, item in data.items():
            meta = NeuronMeta(
                ...
                dtype=DTYPE_MAP.get(item['dtype'], torch.float32),
                device=item['device'],
        ...
        if create_dummy_neurons:
            ...
            if Path(adapter_path).exists():
                ...
                for nid, meta in self.meta_db.items():
                    ...
                    if nid in adapters:
                        try:
                            ...
                            try:
                                ...
                                # Shape migration: zero-pad/interpolate to match target dims
                                def _resize_param(t: torch.Tensor, target_shape: torch.Size) -> torch.Tensor:
                                    if t.shape == target_shape:
                                        return t
                                    try:
                                        # For Linear weights: [out, in]
                                        out_t, in_t = target_shape
                                        t_resized = torch.zeros(out_t, in_t, dtype=t.dtype)
                                        out_s, in_s = t.shape
                                        # Copy overlapping region
                                        t_resized[:min(out_s,out_t), :min(in_s,in_t)] = t[:min(out_s,out_t), :min(in_s,in_t)]
                                        # Simple interpolation on rows if needed
                                        if out_t > out_s and out_s > 0:
                                            repeat = max(1, out_t // out_s)
                                            t_rep = t.repeat(repeat, 1)[:out_t, :min(in_s,in_t)]
                                            t_resized[:, :min(in_s,in_t)] = t_rep
                                        return t_resized
                                    except Exception:
                                        return t
                                def _resize_bias(b: torch.Tensor, target_shape: torch.Size) -> torch.Tensor:
                                    if b.shape == target_shape:
                                        return b
                                    try:
                                        out_t = target_shape[0]
                                        b_resized = torch.zeros(out_t, dtype=b.dtype)
                                        b_resized[:min(out_t, b.shape[0])] = b[:min(out_t, b.shape[0])]
                                        return b_resized
                                    except Exception:
                                        return b
                                # Apply migration for A_in
                                ain_fixed = dict(ain)
                                if hasattr(neuron.A_in, 'weight'):
                                    ain_fixed['weight'] = _resize_param(ain_fixed.get('weight', torch.empty_like(neuron.A_in.weight)), neuron.A_in.weight.shape)
                                if hasattr(neuron.A_in, 'bias') and getattr(neuron.A_in, 'bias') is not None:
                                    ain_fixed['bias'] = _resize_bias(ain_fixed.get('bias', torch.empty_like(neuron.A_in.bias)), neuron.A_in.bias.shape)
                                # Apply migration for A_out
                                aout_fixed = dict(aout)
                                if hasattr(neuron.A_out, 'weight'):
                                    aout_fixed['weight'] = _resize_param(aout_fixed.get('weight', torch.empty_like(neuron.A_out.weight)), neuron.A_out.weight.shape)
                                if hasattr(neuron.A_out, 'bias') and getattr(neuron.A_out, 'bias') is not None:
                                    aout_fixed['bias'] = _resize_bias(aout_fixed.get('bias', torch.empty_like(neuron.A_out.bias)), neuron.A_out.bias.shape)

                                neuron.A_in.load_state_dict(ain_fixed, strict=False)
                                neuron.A_out.load_state_dict(aout_fixed, strict=False)
                    ...
                    # Registra
                    self.neurons[nid] = neuron
                    # FIX: Use string status key consistently
                    status_key = meta.status.value if isinstance(meta.status, Enum) else str(meta.status)
                    if status_key not in self.by_status:
                        self.by_status[status_key] = []
                    self.by_status[status_key].append(nid)
