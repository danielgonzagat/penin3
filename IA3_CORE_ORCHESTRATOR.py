...
class IA3EvolutionEngine:
    ...
    def _generate_random_dna(self) -> str:
        ...
        for mutation in random.sample(mutations, random.randint(0, 2)):
            dna = mutation + dna
    ...
    def _crossover(self, parent1, parent2) -> Dict:
        ...
        if len(dna1) > 0 and len(dna2) > 0:
            crossover_point = random.randint(0, min(len(dna1), len(dna2)))
    ...
    def _mutate_individual(self, individual):
        """Muta um indivíduo"""
        mutation_type = random.choice(['dna', 'capabilities', 'parameters'])
        ...
        if mutation_type == 'dna':
            ...
            new_capability = f"capability_{random.randint(1000,9999)}"
            ...
            individual['mutation_rate'] = random.uniform(0.01, 0.2)
...
class IA3SelfModificationEngine:
    ...
    def _upgrade_to_numpy(self, content: str) -> str:
        ...
        # Substitui random.random() por np.random.random()
...
class IA3CoreOrchestrator:
    ...
    def run_ia3_cycle(self):
        ...
        try:
            ...
            if modifications and random.random() < 0.3:  # 30% chance por ciclo
