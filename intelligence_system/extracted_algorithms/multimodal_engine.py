...
class MultiModalOrchestrator:
    ...
    def activate(self):
        ...
        self.active = True
        # Only log if measurable effect
        uplift = 0.0  # placeholder; real uplift tracked elsewhere
        if uplift > 0:
            logger.info(f"🌈 Multi-modal engine ACTIVATED (+{uplift:.3f})")
        else:
            logger.debug("Multi-modal engine initialized (awaiting first uplift measurement)")
