...
class DatabaseMassIntegrator:
    ...
    def _sanitize_table_name(self, table_name: str) -> str:
        ...
        Args:
            table_name: Raw table name from database
        ...
        # Maximum 64 characters (SQLite limit)
        import re
        ...
        if len(table_name) > 64:
            raise ValueError(f"Table name too long: {len(table_name)} > 64")
        
        # Check for valid characters
        if not re.match(r'^[a-zA-Z0-9_-]+$', table_name):
            raise ValueError(f"Invalid table name: {table_name}")
        
        # Additional safety: wrap in quotes
        return f'"{table_name}"'
    ...
    def inspect_database(self, db_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            for table in tables:
                try:
                    safe_table = self._sanitize_table_name(table)
    ...
    def _extract_evolution_data(self, cursor: sqlite3.Cursor, table: str, source_db: str) -> int:
        ...
        try:
            ...
            if gen_col and fit_col:
                cursor.execute(f"SELECT * FROM {safe_table} LIMIT 100")  # Limit for efficiency
    ...
    def _extract_checkpoint_data(self, cursor: sqlite3.Cursor, table: str, source_db: str) -> int:
        ...
        try:
            safe_table = self._sanitize_table_name(table)
            cursor.execute(f"SELECT * FROM {safe_table} LIMIT 50")  # Checkpoints can be large
            rows = cursor.fetchall()
            cursor.execute(f"PRAGMA table_info({safe_table})")
            columns = [row[1] for row in cursor.fetchall()]
