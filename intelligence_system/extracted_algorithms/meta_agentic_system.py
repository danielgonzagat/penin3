...
async def meta_loop(generations: int, provider_spec: str):
    ...
    try:
        ...
        logger.info(f"{e}  – falling back to open-weights (mistral:7b-instruct.gguf).")
    ...
    for gen in range(generations):
        ...
        logger.info(f"Gen {gen:02d} | acc={fit.accuracy:.3f} lat={fit.latency:.2f}s " f"cost=${fit.cost:.4f} rank={fit.rank}")

    logger.info("✅ Meta-search finished → run  `streamlit run ui/lineage_app.py`")
