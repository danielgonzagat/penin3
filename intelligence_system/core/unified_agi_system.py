...
class PENIN3Orchestrator:
        ...
        while self.running:
            try:
                ...
                if msg['type'] == MessageType.SHUTDOWN.value:
                    ...
                    # 5. PHASE 2: Execute Synergies (every 5 cycles)
    ...
    def compute_meta_metrics(self, v7_metrics: Dict[str, float]) -> Dict[str, float]:
        ...
        # Read meta safely via snapshot to avoid unlocked access
        snapshot = self.unified_state.to_dict()
        o = float(snapshot['meta'].get('omega', 0.0))
        # P0-4b: ensure a minimum omega to allow CAOS amplification to start
        ...
        # Update unified state (thread-safe read of master_I)
    ...
    def evolve_master_equation(self, metrics: Dict[str, float]):
        ...
        # P0-4: boost evolution sensitivity
        delta_linf = metrics.get('linf_score', 0.0) * 100.0
        alpha_omega = 0.5 * metrics.get('caos_amplification', 1.0)
