...
class DatabaseKnowledgeEngine:
    ...
    def __init__(self, db_path: Path):
        ...
        # Ensure schema exists to avoid 'no such table: integrated_data'
        self._ensure_schema()
        try:
            self._load_summary()
        except sqlite3.OperationalError as e:
            logger.warning(f"   ⚠️  integrated_data summary load failed: {e}")
            # Attempt to create schema again and continue
            self._ensure_schema()
    ...
    def _ensure_schema(self):
        """Create required tables/indexes if they do not exist (idempotent)."""
        try:
            self.cursor.execute(
                """
            ...
            # Helpful indexes
            self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_integrated_type ON integrated_data(data_type)")
            self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_integrated_source ON integrated_data(source_db)")
            self.cursor.execute("CREATE INDEX IF NOT EXISTS idx_integrated_time ON integrated_data(timestamp)")
            self.conn.commit()
