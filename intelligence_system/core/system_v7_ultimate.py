...
from extracted_algorithms.darwin_engine_darwinacci import DarwinacciAdapter
...
class IntelligenceSystemV7:
    ...
    def __init__(self):
        ...
        # ⚠️  ARRISCADO: Substituindo por Darwinacci para emergência de inteligência
        self.darwin_real = DarwinacciAdapter(
            population_size=1000,  # População massiva para emergência
            survival_rate=0.1,     # Seleção extrema
            sexual_rate=0.9,       # Reprodução máxima
            mutation_rate=0.8,     # Mutação extrema
            crossover_rate=0.9,    # Crossover máximo
            max_generations=10000  # Gerações ilimitadas
        )
        self.darwin_real.activate()  # FIX C#7: ATIVAR DARWIN!
        self.omega_boost = 0.0  # Omega-directed evolution boost (set by Synergy3)
        
        # ⚠️  PERIGOSO: Configurações de emergência
        if hasattr(self.darwin_real, 'engine'):
            self.darwin_real.engine.mutation_rate = 0.8
            self.darwin_real.engine.crossover_rate = 0.9
            self.darwin_real.engine.population_size = 1000
            self.darwin_real.engine.max_generations = 10000
    ...
    def run_cycle(self):
        ...
        # NEW: Export compact status JSON for external monitors (unified system)
        try:
            import json as _json
            status_out = {
                'cycle': int(self.cycle),
                'ia3': float(results.get('ia3_score', 0.0)),
                'best_mnist': float(self.best.get('mnist', 0.0)),
                'best_cartpole': float(self.best.get('cartpole', 0.0)),
                'darwin_generation': int(getattr(getattr(self, 'darwin_real', None), 'generation', 0)),
            }
            with open('/root/v7_status.json', 'w') as _f:
                _json.dump(status_out, _f)
        except Exception as _e:
            logger.debug(f"status export skipped: {_e}")
    ...
    def _self_modify(self, mnist_metrics: Dict, cartpole_metrics: Dict) -> Dict[str, Any]:
        ...
        applied = 0
        for proposal in proposals[:1]:  # Apply top 1 modification (safely)
            try:
                before_acc = float(mnist_metrics.get('test', 0.0))
                original_model = self.mnist.model
                result = self.self_modifier.apply_modification(
                    model=original_model,
                    modification=proposal
                )
                # Engine may return (model, ok) or bool; support both
                if isinstance(result, tuple) and len(result) == 2:
                    new_model, ok = result
                else:
                    new_model, ok = original_model, bool(result)
                if ok and new_model is not None:
                    self.mnist.model = new_model
                    try:
                        after_acc = float(self.mnist.evaluate())
                    except Exception:
                        after_acc = before_acc
                    if after_acc >= before_acc:
                        applied += 1
                        self._self_mods_applied += 1
                        logger.info(f"   ✅ Applied modification: {proposal.get('operation', 'unknown')} | {before_acc:.2f}% → {after_acc:.2f}%")
                    else:
                        self.mnist.model = original_model
                        logger.info(f"   ↩️  Reverted modification (no gain): {before_acc:.2f}% → {after_acc:.2f}%")
                else:
                    logger.info("   🔧 Modification not applied (engine returned not ok)")
            except Exception as e:
                try:
                    self.mnist.model = original_model  # type: ignore[name-defined]
                except Exception:
                    pass
