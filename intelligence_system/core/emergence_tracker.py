"""
Emergence Tracker - Detecta surprises estatísticas e system connections
"""
import sqlite3
import logging
...
class EmergenceTracker:
    ...
    def _init_dbs(self):
        ...
        try:
            ...
            with sqlite3.connect(self.surprises_db) as conn:
                ...
                """)
                # Diagnostics with lower threshold (>=2σ)
                conn.execute("""
            ...
            logger.info("✅ Emergence tracking DBs initialized")
    ...
    def track_metric(self, metric_name: str, value: float, episode: Optional[int] = None):
        ...
        try:
            ...
            # Record diagnostics for moderate anomalies (>= 2σ)
            if z_score >= 2.0:
                try:
                    with sqlite3.connect(self.surprises_db) as conn:
                        conn.execute(
            ...
            # If >2σ, it's a surprise! (Lowered from 3σ to detect more)
            if z_score > 2.0:
                ...
                logger.warning(
                    f"🌟 SURPRISE DETECTED: {metric_name} = {value:.3f} "
                    f"(expected {mean:.3f}, {z_score:.1f}σ deviation!)"
                )
                # Prometheus and critical alerts for sigma >= 4 (lowered from 5)
                try:
                    if z_score >= 4.0:
                        ...
                        logger.critical(
                            f"🚨 HIGH-SIGMA SURPRISE (>=5σ): metric={metric_name} sigma={z_score:.2f}"
                        )
                except Exception:
                    pass
