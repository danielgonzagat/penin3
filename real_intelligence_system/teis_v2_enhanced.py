...
from common.gymx import gym, make_env
from common.determinism import set_global_seeds as _set_global_seeds
...
# SB3 (Stable-Baselines3) optional integration for real RL training
try:
...
def set_global_seed(seed: int):
    """Compatibility wrapper; delegates to common.determinism."""
...
class RealTaskEngine:
    ...
    """
    def __init__(self, tmp_dir: str, sb3_env_name: str = 'CartPole-v1', sb3_model=None, sb3_env=None):
        ...
        # Optional SB3 model/env (created lazily if not provided)
        self.sb3_env_name = sb3_env_name
    ...
    def select_task(self, agent_state: torch.Tensor, policy='argmax', epsilon=0.1) -> str:
        ...
        if policy == 'argmax' or epsilon <= 0:
            ...
            if np.random.uniform() < epsilon:
                idx = int(np.random.choice(len(TASK_TYPES)))
            else:
                idx = int(np.argmax(probs))
        elif policy == 'entropy_sample':
            idx = int(np.random.choice(len(TASK_TYPES), p=probs / probs.sum()))
        else:
            idx = int(np.argmax(probs))
        return TASK_TYPES[idx]

    def _ensure_sb3(self):
        """Initialize SB3 env/model if available and not yet created."""
        if not SB3_AVAILABLE:
            return False
        try:
            if self.sb3_env is None:
                # Deterministic env creation via shared helper
                env = make_env(self.sb3_env_name, seed=123)
                if Monitor is not None:
                    env = Monitor(env)
                self.sb3_env = env
            if self.sb3_model is None:
                # Small PPO with sane defaults for CartPole
                self.sb3_model = PPO(
                    "MlpPolicy",
                    self.sb3_env,
                    learning_rate=3e-4,
            ...
            # If anything fails, disable SB3 usage for this run
            self.sb3_env = None
            self.sb3_model = None
            return False

    def _sb3_learn_and_eval(self, learn_steps: int = 500, eval_episodes: int = 3) -> Dict[str, Any]:
        """Run a short PPO update and evaluate mean reward deterministically."""
        ok = self._ensure_sb3()
        if not ok:
            return {}
        try:
            # Short update to keep integrated with main loop
            self.sb3_model.learn(total_timesteps=learn_steps)
            mean_reward, std_reward = evaluate_policy(self.sb3_model, self.sb3_env, n_eval_episodes=eval_episodes, deterministic=True, render=False)
            return {
    ...
    def perform_task(self, agent_id: str, agent_state: torch.Tensor) -> Dict[str, Any]:
        ...
        try:
            if task_type == 'file_write':
                ...
                data = [{'v': float(torch.rand(1).item())} for _ in range(100)]
                agg = float(np.mean([d['v'] for d in data]))
                resp = {'type': task_type, 'success': True, 'avg': agg}
            elif task_type == 'gym_cartpole':
                # Prefer SB3-based evaluation for a truthful external metric
                sb3_metrics = self._sb3_learn_and_eval(learn_steps=600, eval_episodes=3) if SB3_AVAILABLE else {}
                if sb3_metrics:
                    mean_rew = sb3_metrics.get('mean_reward', 0.0)
                    resp = {
                        'type': task_type,
                        'success': bool(mean_rew >= 100.0),  # CartPole v1 baseline threshold
                        'reward': float(mean_rew),
                        'max_reward': 500.0,
                        'episodes': sb3_metrics.get('episodes', 3),
                        'std_reward': sb3_metrics.get('std_reward', 0.0)
                    }
                else:
                    # Fallback to simple single-episode rollout using internal task_network
                    env = gym.make('CartPole-v1')
                    try:
                        obs = env.reset()
                        # gymnasium reset returns (obs, info); handle both
...
class RealAgentV2:
    ...
    def decide(self, state: torch.Tensor, exploration='entropy_sample', epsilon=0.05) -> Tuple[int, str, torch.Tensor]:
        ...
        if np.random.rand() < getattr(self, 'incompletude_rate', 0.0):
            action_idx = int(np.random.choice(len(ACTIONS)))
        elif exploration == 'entropy_sample':
            action_idx = int(np.random.choice(len(ACTIONS), p=probs/probs.sum()))
        elif exploration == 'epsilon_greedy':
            if np.random.uniform() < epsilon_eff:
                action_idx = int(np.random.choice(len(ACTIONS)))
            else:
    ...
    def execute(self, action_idx: int, action: str, state: torch.Tensor, environment: Dict[str, Any],
        ...
        if task_engine and self.energy > 30:
            ...
            # Replace heuristic reward by truthful external signal when available
            try:
                if t_out.get('type') == 'gym_cartpole' and 'reward' in t_out:
                    max_rew = float(t_out.get('max_reward', 500.0)) or 500.0
                    ext_norm = float(t_out['reward']) / max_rew
                    # Heavy weight on external reward; keep small portion of internal shaping
                    result['reward'] = 0.25 * result['reward'] + 0.75 * ext_norm
                elif t_out.get('success', False):
                    result['reward'] = max(result['reward'], 1.0)
            except Exception:
                pass
        # Enhanced curiosity-driven exploration: keep as a small shaping term only
        try:
            ...
            if state_sig not in self._novelty_set:
                ...
                novelty_bonus = 0.1 if len(self._novelty_set) < 1000 else 0.05
                ...
                uncertainty_bonus = float(action_entropy) * 0.02
...
class TrueEmergentIntelligenceV2:
    ...
    def __init__(self, num_agents: int = 50, base_dir: str = "./teis_v2_out", seed: int = 747, resume: bool = False):
        ...
        # Safe task engine: exclusive temp dir + optional SB3 PPO (external, truthful reward)
        sb3_model = None
        sb3_env = None
        if SB3_AVAILABLE:
            try:
                ...
                sb3_model, sb3_env = None, None
        self.task_engine = RealTaskEngine(tmp_dir=os.path.join(base_dir, 'tmp'), sb3_env_name='CartPole-v1', sb3_model=sb3_model, sb3_env=sb3_env)
        # Last external evaluation cached for metrics logging
        self._last_external_mean_reward: Optional[float] = None
    ...
    def run_generation(self, exploration='entropy_sample', epsilon=0.20,
        ...
        # ✅ P1 FIX: Aumentar replay 10× para melhorar aprendizado
        replay_multiplier = 10  # Era 2, agora 20 steps total
        for agent in self.agents:
            for _ in range(learn_steps * replay_multiplier):
                agent.learn_from_replay(batch_size=minibatch)

        # Brief external PPO learning step per generation to ensure integration
        try:
            if SB3_AVAILABLE and getattr(self.task_engine, 'sb3_model', None) is not None:
                # Tiny update to keep model improving along generations
                self.task_engine.sb3_model.learn(total_timesteps=400)
                # Evaluate truthful external reward
                mean_rew, _ = evaluate_policy(self.task_engine.sb3_model, self.task_engine.sb3_env, n_eval_episodes=3, deterministic=True, render=False)
                self._last_external_mean_reward = float(mean_rew)
        except Exception:
            self._last_external_mean_reward = None
        ...
        # ✅ P1 FIX: Adaptar difficulty baseado em performance REAL
        try:
            # Calcular success rate médio dos últimos N agents
            recent_success = [a.fitness / max(1, self.generation) for a in self.agents[-10:]]
            avg_success = sum(recent_success) / len(recent_success) if recent_success else 0.5
            
            # Adaptar difficulty dinamicamente
            if avg_success > 0.8:  # Alta performance → aumentar desafio
                self.environment['difficulty'] = min(0.95, self.environment.get('difficulty', 0.5) + 0.05)
            elif avg_success < 0.3:  # Baixa performance → diminuir desafio
                self.environment['difficulty'] = max(0.05, self.environment.get('difficulty', 0.5) - 0.05)
            else:  # Performance OK → manter ou leve aumento
                self.environment['difficulty'] = min(0.90, self.environment.get('difficulty', 0.5) + 0.01)
            
            # Fallback baseado em geração se não houver histórico
            if not hasattr(self, '_difficulty_initialized'):
                if self.generation < 5:
                    self.environment['difficulty'] = 0.05
                elif self.generation < 15:
                    self.environment['difficulty'] = 0.15
                else:
                    self.environment['difficulty'] = min(0.50, 0.05 + (self.generation * 0.01))
    ...
    def log_metrics(self, action_results: List[Dict[str, Any]]):
        ...
        # External, objective metric when available (CartPole mean reward)
        if self._last_external_mean_reward is not None:
            metrics['external_cartpole_mean_reward'] = float(self._last_external_mean_reward)
        try:
