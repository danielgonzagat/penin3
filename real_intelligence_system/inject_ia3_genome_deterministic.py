...
def inject_ia3_genome():
    ...
    for nid, data in neurons_data.items():
        ...
        if nid in individual_genes:
            ...
            architecture = individual_genes[nid].get('architecture', deterministic_choice(architectures))
        else:
            # Distribuir arquiteturas uniformemente
            architecture = deterministic_choice(architectures)
