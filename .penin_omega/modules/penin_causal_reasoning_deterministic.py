...
class CausalReasoningEngine:
    ...
    async def _find_correlations(self):
        ...
        for i in range(min(5, len(self.causal_patterns))):
            correlation = {
                ...
                'strength': deterministic_random(),
                'confidence': deterministic_random()
            }
    ...
    async def _update_world_model(self):
        ...
        for var_id, variable in self.world_model.variables.items():
            ...
            if deterministic_random() < 0.1:  # 10% chance de mudança
                variable.current_value = deterministic_choice(variable.domain)
    ...
    async def _causal_predictions(self):
        ...
        for relation_id, relation in self.world_model.relations.items():
            if relation.confidence > 0.5:
                # Simula predição
                predicted_effect = relation.strength * deterministic_random()
    ...
    async def add_causal_variable(self, name: str, domain: List[Any], 
        ...
        variable = CausalVariable(
            ...
            intervention_cost=deterministic_random(),
            observability=deterministic_random(),
            timestamp=time.time()
        )
...
if __name__ == "__main__":
    ...
    for i in range(100):
        causal_reasoning_engine.add_causal_pattern({
            'temperature': deterministic_randint(0, 5),
            'pressure': deterministic_randint(0, 5),
            'humidity': deterministic_randint(0, 5)
        })
        time.sleep(0.1)
