...
class ReproductionEngine:
    ...
    def __init__(
        ...
        mutation_rate: float = 1.0  # EMERGENCY STORM!
    ):
        """
...
class DarwinOrchestrator:
    ...
    def evolve_generation(
        ...
        *,
        ...
        # Evaluate fitness (with optional deterministic multi-trial averaging)
        for ind in self.population:
            if deterministic:
                # Fixed-seed per individual for reproducibility
                rs = np.random.RandomState(seed + ind.generation + id(ind) % 1000000)
                trials = []
                for t in range(max(1, int(validation_trials))):
                    # Allow fitness_fn to accept an optional random state for determinism
                    try:
                        f = fitness_fn(ind, rs)
                    except TypeError:
                        # Fall back to seeding global RNGs for legacy fitness functions
                        random.seed(seed + t)
                        np.random.seed(seed + t)
                        torch.manual_seed(seed + t)
                        f = fitness_fn(ind)
                    trials.append(float(f))
                base_fitness = float(np.mean(trials))
            else:
                base_fitness = float(fitness_fn(ind))
            novelty_boost = 0.0
        ...
        # Statistics
        fitnesses = [float(ind.fitness) for ind in self.population]
        stats = {
            ...
            'validation_trials': int(validation_trials),
            'seed': int(seed)
        }
